...
const game_offset_secs = -28_800
const game_offset_dst = -25_200
var local_offset_secs = Time.get_time_zone_from_system()["bias"]*60
var server_reset
...
func set_fields():
	...
	server_reset = Time.get_unix_time_from_datetime_dict(datetime) - (game_offset_dst if datetime["dst"] else game_offset_secs)
	...
	$Date.text = "It is %s, %s %s%s, %d"%[days[datetime["weekday"]],months[datetime["month"]],datetime["day"],get_post(str(datetime["day"])),datetime["year"]]
	if type != "None":
		$Current.show()
		for i in range(1,4):
			get_node("Time"+str(i)).show()
			var start = Time.get_time_string_from_unix_time(server_reset + times[c][i-1] + local_offset_secs).replace(":00","")
			var end = Time.get_time_string_from_unix_time(server_reset + times[c][i-1] + local_offset_secs + 14_400).replace(":00","")
...
func get_post(day:String) -> String:
	var post = "th"
